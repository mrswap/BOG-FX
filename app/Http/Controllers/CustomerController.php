...
class CustomerController extends Controller
{
    ...
    public function create()
    {
        ...
        if ($role->hasPermissionTo('customers-add')) {
            ...
            $currencies = Currency::where('is_active', true)->get();  // ✅ Active currencies only
    ...
    public function store(Request $request)
    {
        $this->validate($request, [
            ...
            'base_currency_id' => 'required|exists:currencies,id', // ✅ required validation
        ]);
        ...
        // ✅ Create customer with base_currency_id included
        $lims_customer_data = Customer::create($customer_data);

        // ✅ Custom fields logic unchanged
        $custom_field_data = [];
    ...
    public function edit($id)
    {
        ...
        if ($role->hasPermissionTo('customers-edit')) {
            ...
            return view('backend.customer.edit', compact('lims_customer_data', 'lims_customer_group_all', 'custom_fields', 'currencies'));
        } else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    public function update(Request $request, $id)
    {
        $this->validate($request, [
            ...
            'base_currency_id' => 'required|exists:currencies,id', // ✅ validate currency
        ]);

        $input = $request->all();
        $lims_customer_data = Customer::find($id);

        // If user account is being created from update
        if (isset($input['user'])) {
            $this->validate($request, [
                'name' => [
                    'max:255',
        ...
        // ✅ Update with base_currency_id included
