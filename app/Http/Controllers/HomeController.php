...
class HomeController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        // -----------------------------
        $ledgerBuilder = app(\App\Services\LedgerBuilder::class);
        $ledger = $ledgerBuilder->buildForDataTable([]);
        ...
        foreach ($rows as $r) {
            ...
            $real = (float) $r['realised'];
            $unreal = (float) $r['unrealised'];

            if ($real >= 0) {
                $realisedGain += $real;
            } else {
                $realisedLoss += abs($real);
            }

            if ($unreal >= 0) {
                $unrealisedGain += $unreal;
            } else {
                $unrealisedLoss += abs($unreal);
            }
        }

        // ✅ FINAL, CORRECT GAIN / LOSS
        $totalGainLoss =
